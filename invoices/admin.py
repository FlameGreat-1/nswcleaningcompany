...
class InvoiceItemInline(admin.TabularInline):
    ...
    fields = ['description', 'quantity', 'unit_price', 'total_price', 'is_taxable', 'gst_amount', 'total_with_gst']
...
class InvoiceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('id', 'invoice_number', 'client', 'quote', 'status', 'created_by')
        }),
        ('Dates', {
            'fields': ('invoice_date', 'due_date', 'payment_terms')
        }),
        ('Addresses', {
            'fields': ('billing_address', 'service_address')
        }),
        ('NDIS Information', {
            'fields': ('is_ndis_invoice', 'participant_name', 'ndis_number', 'service_start_date', 'service_end_date'),
            'classes': ('collapse',)
        }),
        ('Financial Details', {
    ...
    def action_buttons(self, obj):
        ...
        buttons.append(f'<a href="{preview_url}" class="button" target="_blank">Preview</a>')
        ...
        if obj.pk:
            ...
            if obj.is_ndis_invoice:
                preview_data.update({
                    ...
                    'service_start_date': obj.service_start_date.strftime('%d/%m/%Y') if obj.service_start_date else '',
                    'service_end_date': obj.service_end_date.strftime('%d/%m/%Y') if obj.service_end_date else ''
                })
    ...
    def generate_invoice(self, request, invoice_id):
        ...
        try:
            ...
            if success:
                messages.success(request, f'PDF generated successfully for invoice {invoice.invoice_number}')
            else:
                messages.error(request, f'Failed to generate PDF for invoice {invoice.invoice_number}')
    ...
    def send_invoice(self, request, invoice_id):
        ...
        try:
            ...
            if success:
                messages.success(request, f'Invoice {invoice.invoice_number} sent successfully to {invoice.client.email}')
            else:
                messages.error(request, f'Failed to send invoice {invoice.invoice_number}')
        except Exception as e:
    ...
    def recalculate_totals(self, request, queryset):
        ...
        messages.success(request, f'Recalculated totals for {queryset.count()} invoices')
    recalculate_totals.short_description = 'Recalculate totals for selected invoices'


@admin.register(InvoiceItem)
class InvoiceItemAdmin(admin.ModelAdmin):
    list_display = ['invoice', 'description', 'quantity', 'unit_price', 'total_price', 'is_taxable']
