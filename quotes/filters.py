...
class QuoteFilter(django_filters.FilterSet):
    ...
    def filter_has_items(self, queryset, name, value):
        if value is True:
            return queryset.annotate(items_count=Count('items')).filter(items_count__gt=0)
        elif value is False:
    ...
    def filter_has_revisions(self, queryset, name, value):
        if value is True:
            return queryset.annotate(revisions_count=Count('revisions')).filter(revisions_count__gt=0)
        elif value is False:
            return queryset.annotate(revisions_count=Count('revisions')).filter(revisions_count=0)
        return queryset
...
class QuoteAttachmentFilter(django_filters.FilterSet):
    ...
    file_size = django_filters.RangeFilter(
        widget=django_filters.widgets.RangeWidget(attrs={
            'class': 'form-control',
            'placeholder': 'Min/Max size (bytes)'
        })
    )
...
class QuoteRevisionFilter(django_filters.FilterSet):
    ...
    previous_price = django_filters.RangeFilter(
        widget=django_filters.widgets.RangeWidget(attrs={
            'class': 'form-control',
            'placeholder': 'Min/Max previous price'
        })
    )
...
class AdvancedQuoteFilter(QuoteFilter):
    ...
    def filter_has_special_requirements(self, queryset, name, value):
        if value is True:
            return queryset.exclude(special_requirements__isnull=True).exclude(special_requirements='')
        elif value is False:
            return queryset.filter(Q(special_requirements__isnull=True) | Q(special_requirements=''))
        return queryset
    
    def filter_has_access_instructions(self, queryset, name, value):
        if value is True:
            return queryset.exclude(access_instructions__isnull=True).exclude(access_instructions='')
        elif value is False:
            return queryset.filter(Q(access_instructions__isnull=True) | Q(access_instructions=''))
        return queryset
...
def get_filter_choices_for_user(user):
    base_choices = {
        'status': Quote.STATUS_CHOICES,
        'cleaning_type': Quote.CLEANING_TYPE_CHOICES,
        'urgency_level': Quote.URGENCY_CHOICES,
