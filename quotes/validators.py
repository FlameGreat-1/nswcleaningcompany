...
def validate_quote_number(value):
    ...
    if int(year_part) > current_year + 1:
        raise ValidationError(
            "Quote number year cannot be more than one year in the future."
        )
...
def validate_preferred_date(value):
    ...
    if value > max_future_date:
        raise ValidationError(
            "Preferred date cannot be more than one year in the future."
        )
...
def validate_ndis_quote_requirements(is_ndis_client, ndis_participant_number, service):
    if is_ndis_client:
        if not ndis_participant_number:
            raise ValidationError(
                "NDIS participant number is required for NDIS clients."
            )

        if (
            service
            and hasattr(service, "is_ndis_eligible")
            and not service.is_ndis_eligible
        ):
...
def validate_service_area_coverage(service, postcode):
    ...
    if (
        hasattr(service, "service_areas")
        and not service.service_areas.filter(postcode=postcode, is_active=True).exists()
    ):
        raise ValidationError(
...
def validate_contact_name(name):
    ...
    if not re.match(r"^[a-zA-Z\s\-\'\.]+$", name):
        raise ValidationError(
            "Contact name can only contain letters, spaces, hyphens, apostrophes, and periods."
        )
...
def validate_quote_source(source):
    ...
    if source not in valid_sources:
        raise ValidationError(
            f"Quote source must be one of: {', '.join(valid_sources)}"
        )
...
def validate_quote_completeness(quote_data):
    ...
    if not quote_data.get("service") and not quote_data.get("service_type"):
...
def validate_bulk_quote_operation(quote_ids, operation):
    ...
    if len(quote_ids) > 100:
        raise ValidationError(
            "Cannot perform bulk operation on more than 100 quotes at once."
        )

    valid_operations = ["approve", "reject", "cancel", "assign", "export"]
    if operation not in valid_operations:
        raise ValidationError(
            f"Invalid bulk operation. Valid operations: {', '.join(valid_operations)}"
        )
...
def validate_quote_approval_requirements(quote):
    ...
    if quote.is_ndis_client and not quote.ndis_participant_number:
        raise ValidationError(
            "NDIS participant number required for NDIS quote approval."
        )
...
def validate_file_upload_permissions(user, quote):
    ...
    if user != quote.client and not user.is_staff:
        raise ValidationError(
            "You don't have permission to upload files for this quote."
        )

    if quote.status in ["cancelled", "converted"]:
        raise ValidationError(
            f"Cannot upload files for quote with status '{quote.status}'."
        )
    ...
    def validate_quote_update(quote, data):
        ...
        if data.get("expires_at"):
            validate_quote_expiry_date(
                data["expires_at"], data.get("status", quote.status)
            )
